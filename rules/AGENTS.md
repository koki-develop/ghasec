...
- `archived-action` is a **non-required online** rule that detects usage of archived GitHub repositories in `uses:` directives. Implements both `WorkflowRule` and `ActionRule`. Uses dependency injection (`ArchiveChecker` interface) for the GitHub API client.
- `mismatched-sha-tag` and `impostor-commit` are **online** rules (require `--online` flag).
- `shellcheck` is a **non-required** rule that runs the external `shellcheck` binary against `run:` shell scripts. Implements both `WorkflowRule` and `ActionRule`. It is unique in several ways: (1) it depends on an external binary, injected via a `Runner` interface (`NewExecRunner()` resolves it with `exec.LookPath`); when unavailable the rule is a no-op and `cmd/root.go` prints a hint via an `atomic.Bool` flag (`SawEligibleStep`). (2) It emits **per-code rule IDs** (`shellcheck/SC2086`) by setting `e.RuleID` itself — the analyzer preserves a pre-set `RuleID` (only assigning `r.ID()` when empty). (3) It sets `diagnostic.Error.Ref` to the ShellCheck wiki URL, overriding the default README link in all renderers. (4) `${{ }}` expressions are masked with byte-length-preserving uppercase variable placeholders (`mask.go`) and findings inside placeholders are dropped (`isInsideMask`). The only code suppressed by name is `SC2154` (variable referenced but not assigned), via `shellcheck -e SC2154`: Actions defines variables outside the analyzed script (`env:`, `$GITHUB_ENV`, matrix, runner env), so it is an unavoidable false positive here. (5) Positions are mapped back to YAML via `rules.ExpressionSpanToken` (`position.go`). See `rules/shellcheck/README.md`.
...
- Online rules (`Online() == true`) require network access and are disabled by default. They run only when `--online` is passed. Currently `archived-action`, `mismatched-sha-tag`, `impostor-commit`, and `unpinned-transitive-action` are online rules.
- New rules: implement `rules.Rule` interface plus `WorkflowRule`, `ActionRule`, or both, and add to the `buildRules()` function in `cmd/root.go`. Online rules should lazily initialize their own dependencies (see `mismatched-sha-tag` for an example). Rule IDs are flat kebab-case names describing the violation they detect (e.g., `invalid-workflow`, `unpinned-action`). Lint rules should also implement `rules.Explainer` (optional interface with `Why()` and `Fix()` methods) to provide markdown output guidance. Structural validation rules (`invalid-workflow`, `invalid-action`, `invalid-expression`) do not implement `Explainer`. Use `rules.CollectStepError`/`CollectStepErrors` and `rules.CollectJobError`/`CollectJobErrors` helpers to implement `CheckWorkflow`/`CheckAction` — they eliminate the boilerplate of iterating steps/jobs and collecting errors. Use `WorkflowMapping.EachJob` for job-level iteration.
