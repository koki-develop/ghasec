...
- Online rules (`Online() == true`) require network access and are disabled by default. They run only when `--online` is passed. Currently `archived-action`, `mismatched-sha-tag`, and `impostor-commit` are online rules.
