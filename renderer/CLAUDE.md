...
- `NewSARIF(ruleList []rules.Rule, version string) *SARIFRenderer` — constructor for the SARIF 2.1.0 renderer. Outputs a single JSON document to stdout. Buffers all results in `PrintParseError`/`PrintDiagnosticError` and flushes in `PrintSummary`. `driver.rules` includes a synthetic `parse-error` entry at index 0 plus all active rules with `helpUri` and optional `shortDescription` from `rules.Explainer.Why()`. See `sarif.go`.
- `PrintParseError(path string, err error) error` — render a YAML parse error (from `goccy/go-yaml`).
- `PrintDiagnosticError(path string, e *diagnostic.Error) error` — render a diagnostic error with source annotation, ancestor breadcrumbs, and rule reference URL. When `diagnostic.Error.Ref` is non-empty it overrides the default `rules/<id>/README.md` URL (used by shellcheck findings, which link to the ShellCheck wiki). The SARIF renderer resolves a `ruleIndex` for unknown rule IDs at print time (`resolveRuleIndex`), appending a `reportingDescriptor` with `Ref` as its `helpUri` — necessary because per-code IDs like `shellcheck/SC2086` are only known during analysis, not at `NewSARIF` construction.
- `PrintSummary(totalFiles, errorCount, errorFileCount, skippedOnline int) error` — render a styled summary block with file counts, error counts, and optional online-rules warning. No-op for `GitHubActionsRenderer`. `MarkdownRenderer` outputs a plain-text summary to stdout. `SARIFRenderer` marshals and writes the complete SARIF JSON document to stdout.
- `PrintHint(message string) error` — render a styled hint message. `DefaultRenderer` outputs yellow text with an info icon to stderr. `GitHubActionsRenderer` emits a `::warning` workflow command to stdout. No-op for `MarkdownRenderer` and `SARIFRenderer`.
